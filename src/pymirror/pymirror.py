    ...
    def _load_modules(self):
        for module_config in self._config.modules:
            ...
            if hasattr(obj, "onKeyboardEvent"):
                self.keyboard.on_event = obj.onKeyboardEvent
                _debug(f"Assigned keyboard input to module {obj._moddef.name}")
        ...
        for module in reversed(self.modules):
            if (not module.disabled) and module.bitmap:
                ...
                if self.focus_module:
                    self._focus_render(self.focus_module)
        self.screen.flush()
